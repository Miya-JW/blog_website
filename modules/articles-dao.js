...
async function retrieveArticleByArticleId(articleId) {
  const db = await database;
  const result = await db.query("select * from articles where articleId=?", [
    articleId,
  ]);
  return result[0];
}
...
async function deleteArticle(articleId) {
  const db = await database;
  await db.query(
    `delete from comment_comment where commentId in (select commentId from comments where articleId = ?) or comment_comment_id in (select commentId from comments where articleId = ?)`,
    [articleId, articleId]
  );
...
async function retrieveAllArticlesById(user_id) {
  const db = await database;
  const result = await db.query(
    "select * from articles where author_id=? ORDER BY date DESC",
    [user_id]
  );
  return result;
}
...
async function retrieveAllArticles() {
  const db = await database;
  const result = await db.query(
    "select articles.*,users.username as author_name from articles join users on articles.author_id =users.user_id"
  );
  return result;
}
...
async function retrieveAllArticlesSorted(sortBy) {
  ...
  const db = await database;
  const result =
    await db.query(`select articles.articleId, articles.title, articles.date, articles.content, articles.image, articles.likes, articles.comments, articles.author_id, users.userName as author_name
...
async function checkIfAuthor(user_id, articleId) {
  const db = await database;
  const result = await db.query(
    "select * from articles where articleId=? and author_id=?",
    [articleId, user_id]
  );
...
async function updateArticleCommentNum(articleId, newCommentNum) {
  const db = await database;
  const result = await db.query(
    "update articles set comments = ? where articleId = ?",
    [newCommentNum, articleId]
  );
  return result.affectedRows > 0;
}
...
async function updateArticleLikeNum(articleId, newLikeNum) {
  const db = await database;
  const result = await db.query(
    "update articles set likes = ? where articleId = ?",
    [newLikeNum, articleId]
  );
  return result.affectedRows > 0;
}
