...
async function retrieveUserIdByUsername(username) {
  const db = await database;
  const result = await db.query("select user_id from users where userName=?", [
    username,
  ]);
  if (result[0]) {
...
async function retrieveUserWithCredentials(username, password) {
  ...
  const rows = await db.query("select password from users where user_id = ?", [
    user_id,
  ]);
  if (rows[0]) {
...
async function deleteUser(user_id) {
  ...
  const userComments = await db.query(
    "select commentId from comments where commenter_id = ?",
    [user_id]
  );

  //---------------------------------For each comment, delete the corresponding comment_comment.-----------------------------------
  ...
  //------------------------------------delete articles-----------------------------------
  const articles = await db.query("select * from articles where author_id=?", [
    user_id,
  ]);
...
async function updateUserProfile(user_id, field, value) {
  ...
  await db.query(`update users set ${field} = ? where user_id = ?`, [
    value,
    user_id,
  ]);
